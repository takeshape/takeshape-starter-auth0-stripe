...
import * as queries from '../queries/takeshape';
import { audience, scope, takeshapeApiKey, takeshapeApiUrl } from '../config';

const createGraphqlRequest = (getAccessToken, query, payload) => {
  return async () => {
    try {
...
export function useQuery(queryName, { useApiKeyAuthentication } = {}) {
  ...
  return useSWR(queryName, createGraphqlRequest(getAccessToken, query));
}
...
export function useMutation(queryName, { initialPayload, useApiKeyAuthentication, revalidateQueryName } = {}) {
  ...
  useEffect(() => {
    async function doQuery() {
      ...
      try {
        const graphqlRequest = createGraphqlRequest(getAccessToken, query, payload);
